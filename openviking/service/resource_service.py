...
_ADD_RESOURCE_ARGS_RESERVED_FIELDS = frozenset(
    {
        ...
        "rollback_lock_handoff_callback",
        "tags",
        "tag_mode",
    }
)
...
_INTERNAL_INGESTION_FIELDS = frozenset(
    {
        ...
        "stage_callback",
        "target_created",
        "target_materialization_pending",
        "watch_auth_state",
        "understanding_response_id",
        "parser_backend",
        "resolved_extension",
...
class ResourceService:
    ...
    async def execute_add_resource_job(
        ...
        *,
        ...
        if msg.prepared is None:
            ...
            if isinstance(result.get("root_uri"), str):
                msg.root_uri = result["root_uri"]
            if isinstance(target_created, bool):
                msg.target_created = target_created if isinstance(target_created, bool) else None
                msg.target_materialization_pending = False
            return result
    ...
    async def cancel_add_resource_task(
        ...
        *,
        ...
        if cancelled is None:
            raise NotFoundError(task_id, "task")
        if cancelled.status not in (TaskStatus.CANCELLING, TaskStatus.CANCELLED):
            raise FailedPreconditionError(
    ...
    async def rollback_cancelled_add_resource(
        ...
        *,
        ...
        if target_created is not True and materialization_pending:
            ...
            if not bool(getattr(msg, "_lock_handoff_adopted", False)):
                raise ResourceBusyError(
                    f"Cancelled target ownership cannot be recovered safely: {msg.root_uri}",
                    uri=msg.root_uri,
            ...
            if target_has_data:
                return
            materialized_lock = await self._resource_processor.materialize_candidate_reservation(
                resource_lock,
                msg.root_uri,
                ctx=ctx,
        ...
        if self._viking_fs is None:
            raise NotInitializedError("VikingFS")
        rollback_lock = materialized_lock
        release_rollback_lock = materialized_lock is not None
        if rollback_lock is None and resource_lock is not None:
            try:
                rollback_lock = await self._viking_fs._async_agfs.pathlock_as_borrowed(
                    resource_lock
                )
            except Exception:
                rollback_lock = None
        if rollback_lock is None:
            dst_path = self._viking_fs._uri_to_path(msg.root_uri, ctx=ctx)
            rollback_lock = await self._viking_fs._async_agfs.pathlock_acquire_tree(
                dst_path,
                timeout_secs=0.0,
            )
            release_rollback_lock = True
        try:
            await self._viking_fs.rm(
                msg.root_uri,
                recursive=True,
                ctx=ctx,
                lease_ref=rollback_lock,
            )
        finally:
            if release_rollback_lock:
                await self._viking_fs._async_agfs.pathlock_release(rollback_lock)
    ...
    async def reacquire_add_resource_job_lock(
        ...
        *,
        ...
        if materialization_pending:
            return await self._viking_fs._async_agfs.pathlock_acquire_exact(
                dst_path,
                timeout_secs=0.0,
            )
        return await self._viking_fs._async_agfs.pathlock_acquire_tree(dst_path, timeout_secs=0.0)
        ...
        **kwargs,
        ...
        try:
            ...
            msg = AddResourceMsg(
                ...
                target_materialization_pending=target_created is None,
    ...
    async def _plan_resource_target(
        ...
        *,
    ...
    ) -> tuple[str, Optional[Dict[str, Any]], bool, Optional[bool]]:
        if not self._resource_processor or not self._viking_fs:
            raise NotInitializedError("ResourceProcessor")
        ...
        if candidate_uri:
            ...
            return root_uri, resource_lock, False, None
        ...
        # Native PathLock may create internal lock directories while acquiring the tree
        # lock, so an empty explicit target cannot be proven task-created. Fail closed:
        # only auto-named candidate reservations grant rollback delete authority.
            ...
            if understanding_source is not None:
                ...
                try:
                    ...
                    msg = AddResourceMsg(
                        ...
                        target_created=target_created,
                        target_materialization_pending=target_created is None
                        and not defer_target_resolution,
                        lock_handoff=lock_handoff,
