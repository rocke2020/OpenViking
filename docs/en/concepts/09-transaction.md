...
record is first persisted as `cancelling`, new DAG work and semantic writes then stop, and in-flight
work drains before rollback. Rollback runs while the lifecycle lock is still held and releases it
afterward; once all task-owned work has settled, the task is persisted as `cancelled`. A deferred
target first persists its resolved URI in a `materialization_pending` state,
