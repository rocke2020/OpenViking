...
持久化为 `cancelling`，再停止调度新的 DAG 工作和语义写入；进行中的工作排空后，系统会在
仍持有生命周期锁时执行回滚，最后才释放锁；任务拥有的工作全部收敛后，状态才会持久化为
`cancelled`。对于延迟解析的目标，系统先把最终 URI
...
也没有任务绑定的 semantic 消息 ID，系统会把它当作旧版非任务工作正常处理；handler
正常返回会 ACK 并删除队列消息，因此缺少新身份字段不能作为安全跳过的理由。升级前
创建的任务没有取消协议标记，cancel 端点会拒绝取消，让其旧版队列工作正常排空。

只有 `target_created=true`，或在精确 reservation lock 下恢复并确认
`materialization_pending` 目标为空时，系统才会删除目标。预先存在的目标，以及无法证明
上述条件的恢复状态，永远不会触发删除。`VikingFS.rm` 会先删除目标的 VectorDB 记录，再
删除文件。索引删除失败，或父目录删除刷新无法持久入队时，队列消息都不会被 ACK，因此
重启恢复可以重试回滚。
