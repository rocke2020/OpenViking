...
@pytest.mark.asyncio
async def test_embedding_handler_does_not_ack_when_tracker_completion_fails(monkeypatch):
    class _CapturingVikingDB:
        is_closing = False
        mode = "local"
    ...
    assert status == {"success": 0, "requeue": 0, "error": 0}
