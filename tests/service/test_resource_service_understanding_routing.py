...
async def test_extensionless_remote_url_queues_frozen_understanding_route(
    ...
    assert queued.target_created is None
    assert queued.target_materialization_pending is True
    assert not downloaded.exists()
    processor.submit_understanding.assert_awaited_once_with(prepared)
    processor.reserve_unique_candidate.assert_awaited_once_with(
        candidate_uri="viking://resources/manual",
        ctx=ctx,
        create_target=False,
    )
