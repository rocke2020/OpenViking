...
@pytest.mark.asyncio
async def test_vectorize_file_propagates_rejected_work_tracker_completion_failure(monkeypatch):
    ...
    tracker = types.SimpleNamespace(decrement=AsyncMock(side_effect=RuntimeError("release failed")))
    monkeypatch.setattr(embedding_utils, "get_queue_manager", lambda: DummyQueueManager(queue))
    monkeypatch.setattr(embedding_utils, "get_viking_fs", lambda: DummyFS("hello"))
    monkeypatch.setattr(
        embedding_utils,
    ...
    monkeypatch.setattr(EmbeddingTaskTracker, "get_instance", lambda: tracker)
    ...
    tracker.decrement.assert_awaited_once_with("semantic-1")
...
@pytest.mark.asyncio
async def test_vectorize_file_propagates_source_task_id(monkeypatch):
    queue = DummyQueue()
    monkeypatch.setattr(embedding_utils, "get_queue_manager", lambda: DummyQueueManager(queue))
    monkeypatch.setattr(embedding_utils, "get_viking_fs", lambda: DummyFS("source content"))
    monkeypatch.setattr(
        embedding_utils,
        "get_openviking_config",
        lambda: types.SimpleNamespace(
    ...
    await embedding_utils.vectorize_file(
        file_path="viking://resources/test.md",
    ...
    assert len(queue.items) == 1
    assert queue.items[0].source_task_id == "task-1"
...
@pytest.mark.asyncio
async def test_vectorize_directory_propagates_source_task_id(monkeypatch):
    queue = DummyQueue()
    monkeypatch.setattr(embedding_utils, "get_queue_manager", lambda: DummyQueueManager(queue))
    monkeypatch.setattr(embedding_utils, "get_viking_fs", lambda: DummyFS("ignored"))
    ...
    assert len(queue.items) == 2
    assert all(item.source_task_id == "task-1" for item in queue.items)
